...
2. **Versionless API × versioned API hybrid** (`src/versionless_api.rs`)
   — logic only; a real dependency on RPoem's existing VersionlessAPI
   implementation has not been wired up yet.
...
"rust-analyzer pattern" used by that project). Verified that the
...
- RPoem's VersionlessAPI implementation is not yet wired in as a real
  dependency.
- None of the 5 AI providers (Claude/ChatGPT/Gemini/DeepSeek/Grok)
  have been connectivity-tested with a real API key.
- Only 1 of the 5 LSP custom requests (`cleanupAdvice`) has a
  corresponding UI affordance in the VS Code extension.
